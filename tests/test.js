...
describe('webfont', () => {
	...
	afterEach(() => {
		const files = fs.readdirSync(DEST).map(file => path.join(DEST, file));
	...
	describe('custom templates', function () {
		...
		const RENDERED_TEMPLATE = 'custom template ' + TEMPLATE_OPTIONS.option + EOL;

		it('uses custom css template', done => {
